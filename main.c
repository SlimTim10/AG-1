...
enum ButtonPressType wait_for_button_release(void);
// old
void LED1_DOT(void);
void LED1_DASH(void);
...
struct AccelSample accel_samples[RAW_BUFF_SIZE];
...
enum DeviceState init(void) {
	...
	construct_accel_sample_buffer(&accel_sample_buffer, accel_samples, RAW_BUFF_SIZE);
...
void button_press_event(void) {
	...
	enum ButtonPressType button_press_type = wait_for_button_release();
}
